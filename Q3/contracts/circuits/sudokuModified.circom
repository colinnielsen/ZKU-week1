...
template sudoku() {
    ...
    component proofs[9][9][2];

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            proofs[i][j][0] = RangeProof(4);

            proofs[i][j][0].in <== puzzle[i][j];
            proofs[i][j][0].range[0] <== 0;
            proofs[i][j][0].range[1] <== 9;

            proofs[i][j][1] = RangeProof(4);

            proofs[i][j][1].in <== solution[i][j];
            proofs[i][j][1].range[0] <== 0;
            proofs[i][j][1].range[1] <== 9;
